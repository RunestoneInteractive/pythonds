...
5x5 the tree will be 25 levels deep, or :math:`N = 24` counting the first level
...
speeds up the ``knight_tour``. This function (see :ref:`Listing 4 <lst_avail>`), called ``order_by_avail``
will be used in place of the call to ``u.get_neighbors`` in the code previously
shown above. The critical line in the
``order_by_avail`` function is line 10. This line ensures that we
