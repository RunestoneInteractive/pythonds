...
for each vertex in the graph. In ``dfs_visit`` the loop is executed once for each edge in the adjacency
list of the current vertex. Since ``dfs_visit`` is only called
